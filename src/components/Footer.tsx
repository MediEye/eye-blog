import Link from 'next/link'

export default function Footer() {
  return (
    <footer className="bg-gray-800 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {/* Brand */}
          <div className="col-span-1 md:col-span-2">
            <div className="flex items-center mb-4">
              <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center">
                <span className="text-white font-bold text-sm">E</span>
              </div>
              <span className="ml-2 text-xl font-bold">Eye-Blog</span>
            </div>
            <p className="text-gray-300 mb-4">
              眼科医、視能訓練士、眼科看護師向けの眼科論文専門ブログサイトです。
              最新の眼科研究や臨床知見を共有し、眼科医療の発展に貢献します。
            </p>
            <div className="flex space-x-4">
              <a href="#" className="text-gray-300 hover:text-white">
                <span className="sr-only">Twitter</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                </svg>
              </a>
              <a href="#" className="text-gray-300 hover:text-white">
                <span className="sr-only">Facebook</span>
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                  <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-lg font-semibold mb-4">クイックリンク</h3>
            <ul className="space-y-2">
              <li>
                <Link href="/papers" className="text-gray-300 hover:text-white">
                  論文一覧
                </Link>
              </li>
              <li>
                <Link href="/submit" className="text-gray-300 hover:text-white">
                  論文投稿
                </Link>
              </li>
              <li>
                <Link href="/bookmarks" className="text-gray-300 hover:text-white">
                  ブックマーク
                </Link>
              </li>
              <li>
                <Link href="/about" className="text-gray-300 hover:text-white">
                  サイトについて
                </Link>
              </li>
            </ul>
          </div>

          {/* Categories */}
          <div>
            <h3 className="text-lg font-semibold mb-4">カテゴリー</h3>
            <ul className="space-y-2">
              <li>
                <Link href="/category/cataract" className="text-gray-300 hover:text-white">
                  白内障
                </Link>
              </li>
              <li>
                <Link href="/category/glaucoma" className="text-gray-300 hover:text-white">
                  緑内障
                </Link>
              </li>
              <li>
                <Link href="/category/retinal" className="text-gray-300 hover:text-white">
                  網膜疾患
                </Link>
              </li>
              <li>
                <Link href="/category/corneal" className="text-gray-300 hover:text-white">
                  角膜疾患
                </Link>
              </li>
            </ul>
          </div>
        </div>

        <div className="border-t border-gray-700 mt-8 pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-gray-300 text-sm">
              © 2024 Eye-Blog. All rights reserved.
            </p>
            <div className="flex space-x-6 mt-4 md:mt-0">
              <Link href="/privacy" className="text-gray-300 hover:text-white text-sm">
                プライバシーポリシー
              </Link>
              <Link href="/terms" className="text-gray-300 hover:text-white text-sm">
                利用規約
              </Link>
              <Link href="/contact" className="text-gray-300 hover:text-white text-sm">
                お問い合わせ
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  )
} 